...
func initCli() {
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.dockvisor.yaml, ~/.config/dockvisor/*.yaml, or $XDG_CONFIG_HOME/dockvisor.yaml)")
	rootCmd.PersistentFlags().String("source", "docker", "The container engine to fetch the image from. Allowed values: "+strings.Join(dockvisor.ImageSources, ", "))
	...
	rootCmd.Flags().StringVar(&ciConfigFile, "ci-config", ".dockvisor-ci", "If CI=true in the environment, use the given yaml to drive validation rules.")
...
func initConfig() {
	...
	viper.SetDefault("log.path", "./dockvisor.log")
...
// and in $HOME/.config/dockvisor/ directory
// defaults to $HOME/.dockvisor.yaml
func getDefaultCfgFile() string {
	...
	return path.Join(home, ".dockvisor.yaml")
}

// findInPath returns first "*.yaml" file in path's subdirectory "dockvisor"
// if not found returns empty string
func findInPath(pathTo string) string {
	directory := path.Join(pathTo, "dockvisor")
